...
| `evgen/` | `polligen`: the doubly polarized e+⁶,⁷Li event generator (the first of its kind) — spin-density kernel, inclusive + tagged samplers, run-plan bookkeeping, estimators, the coherent intact-⁶Li channel, money plots 4–7 and their reconstructed-level versions 5R/7R/6R, the hadronic-final-state layer with the hadron-side detector response (`polligen/hfs.py`; PYTHIA 8 sample via `tools/pythia8`) |
